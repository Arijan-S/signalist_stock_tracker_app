...
// Local helpers (were previously incorrectly imported from ../utils)
...
function validateArticle(article: RawNewsArticle | undefined | null): boolean {
  if (!article) return false;
  const { id, headline, url, datetime } = article;
  return (
    typeof id === "number" &&
...
function formatArticle(
  ...
  return {
    ...
    category: article.category || (isCompanyNews ? "company-news" : "general"),
    related: article.related || (symbol ? symbol.toUpperCase() : ""),
    image: article.image,
  };
}

import { POPULAR_STOCK_SYMBOLS } from "../constats";
...
export const searchStocks = cache(
  async (
    ...
    try {
      ...
      // Get user's watchlist symbols if email provided
